 ...
 */
typedef StatePool<T> = Map<String, Pool<FlxFSMState<T>>>

/**
 * A generic Finite-state machine implementation.
 */
class FlxFSM<T> implements IDestroyable
{
	/**
	...
	public function update(elapsed:Float):Void
	{
		...
		if (transitions != null && pools != null)
		{
			...
			if (newStateClass != stateClass)
			{
				...
				{
					pools.set(newName, new Pool<FlxFSMState<T>>(newStateClass));
				}
 ...
 */
class FlxFSMStack<T> extends FlxFSMStackSignal implements IDestroyable
{
	/**
...
private class FlxFSMStackSignal
{
	...
	private static var _lockSignal:Signal1<Int>;
	
	public function new()
	{
		if (FlxFSMStackSignal._lockSignal == null)
		{
			FlxFSMStackSignal._lockSignal = new Signal1<Int>();
		}
	}
